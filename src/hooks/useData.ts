...
export function useDestinationsData() {
  const [items, setItems] = useState<Destination[]>(() =>
    storage.loadDestinations(defaultDestinations)
  );
  ...
  const update = useCallback(
    (id: number, patch: Partial<Destination>) => {
...
export function useFoodCategoriesData() {
  const [items, setItems] = useState<FoodCategory[]>(() =>
    storage.loadFoodCategories(defaultFoodCategories)
  );
  ...
  const update = useCallback(
    (id: string, patch: Partial<FoodCategory>) => {
...
export function useDistrictsData() {
  const [items, setItems] = useState<District[]>(() =>
    storage.loadDistricts(defaultDistricts)
  );
  ...
  const update = useCallback(
    (id: string, patch: Partial<District>) => {
      persist(items.map((d) => (d.id === id ? { ...d, ...patch } : d)));
    },
    [items, persist]
  );
