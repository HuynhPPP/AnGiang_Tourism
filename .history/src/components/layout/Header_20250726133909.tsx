        ...
        <div className="flex flex-col md:flex-row items-center justify-between">
          <div className="flex-shrink-0 mb-4 md:mb-0 flex items-center">
            <img 
              src="/images/Logo.jpg" 
              alt="Logo Thốt Nốt" 
              className="h-14 w-14 mr-3 rounded-full border-2 border-amber-200 object-cover"
            />
            <Link to="/" className="flex items-center">
              <span className="text-2xl font-bold text-amber-50">Du Lịch An Giang</span>
            </Link>
          </div>
          
          <nav className="flex items-center space-x-1 bg-amber-900/50 px-2 py-1 rounded-full backdrop-blur-sm">
            <Link to="/">
              <Button variant={isActive('/') ? "secondary" : "ghost"} className="text-amber-50 hover:text-amber-100">
                Điểm Đến
              </Button>
            </Link>
            <Link to="/about">
              <Button variant={isActive('/about') ? "secondary" : "ghost"} className="text-amber-50 hover:text-amber-100">
                Giới Thiệu
              </Button>
            </Link>
            <Link to="/food">
              <Button variant={isActive('/food') ? "secondary" : "ghost"} className="text-amber-50 hover:text-amber-100">
                Ẩm Thực
              </Button>
            </Link>
            <Link to="/districts">
              <Button variant={isActive('/districts') ? "secondary" : "ghost"} className="text-amber-50 hover:text-amber-100">
                Huyện/Thị
              </Button>
            </Link>
          </nav>
